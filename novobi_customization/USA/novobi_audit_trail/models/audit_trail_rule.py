...
class AuditTrailRule(models.Model):
    ...
    def _monkey_patch_method(self, tracking_model, method_name):
        ...
        def tracking_create_operation(self, vals):
            ...
            self.env['audit.trail.rule'].sudo(self.env.uid).create_audit_trail_log('create', res, new_values=new_values)
            return res
        ...
        def tracking_write_operation(self, vals):
            ...
            self.env['audit.trail.rule'].sudo(self.env.uid).create_audit_trail_log('write', self, old_values,
                                                                                   new_values)
            return res
        
        @api.multi
        def tracking_unlink_operation(self):
            self.env['audit.trail.rule'].sudo(self.env.uid).create_audit_trail_log('unlink', self)
    ...
    def create_audit_trail_log(self, operation, records, old_values={}, new_values={}):
        ...
        for record in records:
            ...
            if operation == 'unlink':
                ...
                if not new_state and not old_state and getattr(record, 'state',
                                                               '') in ignore_states and operation == 'write':
                    continue
    ...
    def create_action_view_audit_log(self):
        ...
        for rule in self:
            domain = "['|', '&', ('model_id', '=', {}), ('res_id', 'in', active_ids), '&', ('parent_model_id', '=', active_model), ('parent_id', 'in', active_ids)]".format(
                rule.model_id.id)
            vals = {
                'name': _('Audit Logs'),
