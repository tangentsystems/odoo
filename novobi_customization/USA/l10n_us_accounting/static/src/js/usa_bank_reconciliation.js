...
var usa_bank_reconciliation = account_report.extend({
    ...
    bank_reconcile: function(e) {
        ...
        var batch_ids = [];
        _.each(this.$el.find("tbody input[checkbox-model='account.batch.payment']"), function(e) {
            batch_ids.push(parseInt(e.attributes['checkbox-id'].value));
        });

        return this._rpc({
                model: 'account.bank.reconciliation.data',
                method: 'check_difference_amount',
                args: [[parseInt(data_id)], aml_ids, batch_ids,
    ...
    _massUpdate: function (checked) {
        ...
        var batch_ids = [];
        _.each(self.$el.find("td[data-model^='account.batch.payment']"), function(e) {
            batch_ids.push($(e).data('id'));
        });

        var msg = "Are you sure you want to <strong>select</strong> all transactions?"
        if (checked === false) {
        ...
        new Dialog(this, {
            ...
            buttons: [
                {text: _t('OK'), classes : "btn-primary", click: function() {
                    ...
                    self._updateTemporaryReconciled('account.batch.payment', batch_ids, checked);
                }, close:true},
                {text: _t("Cancel"), click: function() {
                }, close: true}
            ]
    ...
    _onSelectItem: function(event) {
        ...
        // Update to model
        var target_id = $(event.target).parents('tr').find('td[data-id]').data('id');
        var target_model = $(event.target).parents('tr').find('td[data-model]').data('model');
    ...
    _updateAmount: function() {
        var cleared_deposits = 0, cleared_payments = 0, no_payments = 0, no_deposits = 0;
