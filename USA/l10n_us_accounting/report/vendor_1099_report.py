...
class report_vendor_1099(models.AbstractModel):
    ...
    def _get_result_lines(self, options):
        ...
        query = """
            SELECT partner_odoo_id, partner_name, partner_ssn, partner_company_ssn, partner_address,
    ...
    def _get_move_lines_query_statement(self, options, params={}):
        ...
        query_stmt = """
                        SELECT distinct(aml.id) as line_id,
                           ...
                           (move_line.reconciled_amount / am.amount) as matched_percentage
                                      
                        FROM (
                           SELECT move_line.move_line_id, SUM(amount) as reconciled_amount
                           FROM (
                                 SELECT apr.credit_move_id as move_line_id, apr.amount as amount
                                 ...
                                 WHERE am2.state = 'posted'
                                      ...
                                      AND (apm.id NOT IN {} OR apm.id IS NULL)
                                 UNION
                                 SELECT apr.debit_move_id as move_line_id, apr.amount as amount
                                 FROM account_partial_reconcile apr
                                 ...
                                 WHERE am2.state = 'posted'
                                      ...
                                      AND (apm.id NOT IN {} OR apm.id IS NULL)
                                 UNION
                                 SELECT aml4.id as move_line_id, am2.amount as amount
                           ...
                           GROUP BY move_line.move_line_id
                        ) as move_line
                        ...
                        WHERE am.state = 'posted'
                             ...
                             AND aml.date >= '{}' AND aml.date <= '{}'
                             AND partner.vendor_eligible_1099 = True
                             AND aml.company_id IN ({})
                             {}
                             {}
                        """.format(untrack_payment_methods, untrack_payment_methods, untrack_payment_methods,
                                   date_from, date_to, ','.join(str(company) for company in company_ids),
                                   partner_filter, eligible_filter)
        return query_stmt
